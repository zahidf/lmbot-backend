...
class TriageSubmissionRequest(BaseModel):
    ...
    burner_series: Optional[str] = Field(
        None, description="Burner series: FD, TX, DB, FDB or null if unknown"
    )
    ...
    serial_number: Optional[str] = Field(
        None, description="Burner serial number, e.g. J123456"
    )
    has_serial_number: bool = Field(
        False, description="Whether the user has a serial number"
    )

    # Q3: Issue category
    issue_category: str = Field(..., description="Issue category code: A-G")
    issue_free_text: Optional[str] = Field(
        None, description="Free text description (used when issue_category is G)"
    )
